...
export default {
  ...
  methods: {
    ...
    getGitHubData() {
      const userinfo = session.get('userinfo', { decode: true });
      const [repos] = splitRepos(session.get('repos', { decode: true }));

      this.Followers = userinfo.followers;
      this.Repositories = repos.length;
      this.Forks = repos.reduce((prev, item, index, arr) => prev + item.forks_count, 0);
      this.Stars = repos.reduce((prev, item, index, arr) => prev + item.stargazers_count, 0);
    }
  },
  components: {
    Gauge
